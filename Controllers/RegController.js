...
const finalVerification = async (email) => {
...
const register = async (req, res) => {
  ...
  if (error) {
    ...
    try {
      ...
      return res.status(StatusCodes.CREATED).json({ name: user.firstName });
    } catch (error) {
...
const loginController = async (req, res) => {
  ...
  res.header("authorization-token", token).json({
    msg: "Success!",
    verification:
      "Please enter the code sent to the email associated with your account.",
  });
};
